...
function sessionFiles(root: string, current = root): string[] {
  return fs.readdirSync(current, { withFileTypes: true }).flatMap((entry) => {
    ...
    if (!entry.isFile() || relative === LIVE_SESSION_MIGRATION_FILE || relative === 'logical-pipeline.json'
...
export function prepareLiveSessionMigration(
  ...
  const payload = {
    ...
    preserved_artifacts: inventory(sessionDir),
...
export function verifyLiveSessionMigration(sessionDir: string, migration: InstalledRuntimeMigration): void {
  ...
  const current = new Map(inventory(sessionDir).map((entry) => [entry.path, entry]));
