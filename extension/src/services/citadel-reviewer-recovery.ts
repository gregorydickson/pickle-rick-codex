...
const MECHANISMS: RecoveryMechanism[] = ['schema_scaffold_replay', 'evidence_bundle_reconstruction'];
...
function selectMechanism(state: ReviewState, used: Set<RecoveryMechanism>): RecoveryMechanism | null {
  ...
  const preferred = /schema|json|field|key|array|required|coverage/i.test(failures)
    ? MECHANISMS
    : [...MECHANISMS].reverse();
  return preferred.find((mechanism) => !used.has(mechanism)) ?? null;
}
...
export async function repairCitadelReviewerArtifactContract(
  ...
  const mechanism = resumableMechanism ?? selectMechanism(state, usedMechanisms);
  if (!mechanism) {
    return {
  ...
  if (!journal || journal.diagnostic_identity !== diagnosticIdentity) {
    journal = {
      ...
      failed_candidate_hashes: hashes,
      validator_invariants: [...VALIDATOR_INVARIANTS],
      mechanism_history: [...usedMechanisms, mechanism],
      status: 'started',
      attempts: [],
  ...
  if (interrupted?.status === 'started' && fs.existsSync(interrupted.candidate_path)) {
    try {
      ...
      const runtimeArtifacts = materializeRuntimeArtifacts(sessionDir, state, block, artifact);
