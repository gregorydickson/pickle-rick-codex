...
export function normalizeManifestTicketIds(manifest) {
  ...
  manifest.tickets = manifest.tickets.map((ticket, index) => {
    ...
    if (Array.isArray(ticket.depends_on)) {
      const nextDependsOn = ticket.depends_on
        .map((value) => idRewrites.get(String(value)) || normalizeTicketId(value, String(value)))
        .filter((value) => value && !isNoneDependency(value));
      if (JSON.stringify(nextDependsOn) !== JSON.stringify(ticket.depends_on)) {
        changed = true;
        nextTicket = { ...nextTicket, depends_on: nextDependsOn };
      }
    } else if (typeof ticket.depends_on === 'string' && ticket.depends_on && !isNoneDependency(ticket.depends_on)) {
...
export function ticketDependencyIds(ticket) {
  ...
  if (Array.isArray(dependencyField)) {
    return dependencyField
      ...
      .filter((id) => id && !isNoneDependency(id));
  }
  if (typeof dependencyField === 'string' && dependencyField) {
    const dependencyId = normalizeTicketId(dependencyField, '');
    if (dependencyId && !isNoneDependency(dependencyId)) {
      return [dependencyId];
    }
  }
  return [];
...
function fileTicketsCoverManifest(fileTickets, manifestTickets) {
  ...
  return manifestTickets.every((ticket, index) => {
    const ticketId = canonicalTicketId(ticket, index);
    const fileTicket = fileTicketsById.get(ticketId);
    if (!fileTicket) return false;
    return fileTicket.content === renderTicketFileContent(ticket, index + 1, ticketId);
  });
}
...
function pruneObsoleteTicketFiles(fileTickets, manifestTickets) {
  ...
  for (const ticket of fileTickets) {
    const ticketId = normalizeTicketId(ticket.id, path.basename(path.dirname(ticket.filePath)));
    if (validTicketIds.has(ticketId)) {
      continue;
    }
    fs.rmSync(ticket.filePath, { force: true });
    try {
      fs.rmdirSync(path.dirname(ticket.filePath));
    } catch {
      // Keep ticket directories that still contain artifacts.
    }
  }
}
...
function contractDecisionArtifacts(ticket) {
  ...
  for (const artifactPath of ticketVerificationCommands(ticket).flatMap((command) => extractRepoRelativePaths(command)).filter(isGeneratedArtifactPath)) {
...
export function validateRefinementManifest(manifest) {
  ...
  for (const ticket of tickets) {
    ...
    for (const artifactPath of contractDecisionArtifacts(ticket)) {
      const owners = contractDecisionByArtifact.get(artifactPath) || [];
      owners.push(ticket.id || ticket.title || 'ticket');
      contractDecisionByArtifact.set(artifactPath, owners);
    }
  ...
  tickets.forEach((ticket, index) => {
    ...
    for (const artifactPath of referencedPaths.filter(isGeneratedArtifactPath)) {
      ...
      const hasAmbiguousFreezeContract = (
        ticket.freeze_contract?.artifact_path === artifactPath
        && !hasExplicitFreezeShaSource(ticket.freeze_contract)
      );
      if (
        /freeze/i.test(artifactPath)
        && !authoritativeFreezeByArtifact.has(artifactPath)
        && !contractDecisionByArtifact.has(artifactPath)
        && !hasAmbiguousFreezeContract
      ) {
    ...
    if (ticket.freeze_contract?.artifact_path) {
      const artifactPath = ticket.freeze_contract.artifact_path;
      if (!hasExplicitFreezeShaSource(ticket.freeze_contract)) {
        if (!contractDecisionByArtifact.has(artifactPath)) {
          issues.push(
            `${label}: ambiguous evolving external contract for "${artifactPath}" requires an explicit contract-decision ticket instead of implicit fixed-SHA enforcement`,
          );
        }
        return;
      }
