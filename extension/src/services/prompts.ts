...
export function buildRefinementSynthesisPrompt({
  ...
  return [
    ...
    'Use allowed_paths for implementation scope. output_artifacts and proof_corpus are optional: omit them unless they are literal, durable paths exclusively owned by this one ticket. Never use either field to list a shared source directory or a later ticket’s test directory.',
...
export function buildLoopPrompt({
  ...
  if (mode === 'microverse' && loopConfig.experiment_id) {
    ...
    const prompt = [
      ...
      `Before modifying the repository, write ${loopConfig.experiment_artifact_path || path.join(artifactDir, 'microverse-experiment.json')} with keys: experiment_id (exactly ${loopConfig.experiment_id}); non-empty string hypothesis, hypothesis_family, differentiator, rationale; non-empty string[] target_paths. Inspect this path first. If the handoff marks the plan frozen, the runtime has preseeded the artifact: preserve those plan fields exactly and add only completion evidence. Otherwise define the plan before changing the repository. Before returning the promise token, add insight as a non-empty string and verification as a non-empty string[] of plain command/result summaries—never objects. This is the only measured-experiment output artifact.`,
      ...
      'The experiment target_paths are an immutable write contract: change no repository path outside them. Within a target path, preserve unrelated existing content verbatim; do not bulk-format, regenerate, or repair pre-existing material unless that material is explicitly part of this experiment’s hypothesis. The runtime archives and rolls back out-of-scope changes.',
