...
- `bin/pickle-tmux.js` — INVARIANT: resume must demote dead tmux-runner state to `runner_lost` before relaunch. BREAKS: detached restarts erase crash evidence and stale runner ownership. ENFORCE: `tests/session-flow.test.js` pickle-tmux runner-loss resume case. PATTERN_SHAPE: resume path checking `tmux_runner_pid` liveness without persisting abrupt-loss state.
