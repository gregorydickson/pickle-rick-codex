...
- `bin/loop-runner.js` — INVARIANT: anatomy fallback commits must stage new regression files, not only tracked paths. BREAKS: successful fix iterations leave uncommitted tests and a dirty tree. ENFORCE: `tests/session-flow.test.js` new-regression-file auto-commit case. PATTERN_SHAPE: detached loop auto-commit path using tracked-only staging after a no-commit anatomy iteration.; INVARIANT: `TASK_COMPLETED` marks an iteration boundary, not loop convergence. BREAKS: detached anatomy iterations stop before reaching convergence. ENFORCE: `tests/session-flow.test.js` task-completed loop contract case. PATTERN_SHAPE: loop exit gate treating `TASK_COMPLETED` as equivalent to `LOOP_COMPLETE`.
- `bin/pickle-tmux.js` — INVARIANT: resume must demote dead tmux-runner state to `runner_lost` before relaunch. BREAKS: detached restarts erase crash evidence and stale runner ownership. ENFORCE: `tests/session-flow.test.js` pickle-tmux runner-loss resume case. PATTERN_SHAPE: resume path checking `tmux_runner_pid` liveness without persisting abrupt-loss state.; INVARIANT: resolve `--resume` to a session before runner-loss mutation. BREAKS: bare resume crashes on null session state. ENFORCE: `tests/session-flow.test.js` no-session resume case. PATTERN_SHAPE: resume flow mutating state before null-checking `resolveBootstrapResumeSessionDir()`.
...
- `lib/session-map.js` — INVARIANT: stale session-map locks are stealable only when the recorded owner pid is dead. BREAKS: concurrent writers can overwrite cwd→session mappings after a live lock ages past the stale window. ENFORCE: `tests/session-flow.test.js` live-owner stale-lock case. PATTERN_SHAPE: session-map lock cleanup keyed only to file age without owner liveness.
- `lib/state-manager.js` — INVARIANT: legacy or corrupt stale-looking state locks are stealable only when the recorded pid is dead. BREAKS: concurrent writers can overwrite session or pipeline state under a live owner. ENFORCE: `tests/state-manager.test.js` live-owner legacy-lock case. PATTERN_SHAPE: stale lock fallback deleting by mtime without checking the legacy pid payload.
