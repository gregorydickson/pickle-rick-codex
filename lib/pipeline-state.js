...
function resolveRewrittenVitestCommand(tokens, cwd) {
...
function readPackageName(packageDir) {
  try {
    ...
    return typeof parsed?.name === 'string' && parsed.name.trim() ? parsed.name.trim() : '';
  } catch {
    return '';
  }
}
...
function resolveVerificationCommandCwd(tokens, cwd) {
...
function extractRewrittenVitestTargets(tokens, cwd) {
  const rewrittenVitest = resolveRewrittenVitestCommand(tokens, cwd);
  if (!rewrittenVitest) {
    return [];
  }
  ...
  for (let index = rewrittenVitest.runIndex + 1; index < tokens.length; index += 1) {
    ...
    targets.push(normalizeComparablePath(path.resolve(rewrittenVitest.commandCwd, token), cwd));
  }
...
function parseVitestFailures(output, cwd, scope, commandCwd = cwd) {
...
function buildStoredVerificationScopeKey(kind, targets, fallbackKey) {
...
function normalizeVerificationBaselineCommandMap(value, cwd = process.cwd()) {
  ...
  for (const [scopeKey, entry] of Object.entries(value)) {
    ...
    const storedTargets = Array.isArray(entry.scope?.targets) ? entry.scope.targets.filter(Boolean) : [];
    const storedKind = typeof entry.scope?.kind === 'string' ? entry.scope.kind : null;
    const normalizedKind = storedKind === 'node-test' || storedKind === 'package-test'
      ? storedKind
...
function normalizeVerificationBaselines(value, cwd = process.cwd()) {
  if (!value || typeof value !== 'object' || Array.isArray(value)) {
    return {
...
export function validatePipelineState(pipeline, pipelineState) {
  ...
  return {
    ...
    verification_baselines: normalizeVerificationBaselines(pipelineState.verification_baselines, contract.working_dir),
  };
}
